...
@Component({
  selector: 'app-internet-banking-quick',
  templateUrl: './internet-banking-quick.component.html',
  styleUrls: ['./internet-banking-quick.component.scss']
})
export class InternetBankingQuickComponent implements OnInit, DoCheck {
  oldUrl = null;
  yinlian;
  isCompany = 0;
  isDiscounts;
  ...
  ngDoCheck(): void {
    ...
    if (this.oldUrl !== this.$GlobalService.globalQueryModel.getPayListData) {
      ...
      if (Object.keys(this.oldUrl).length !== 0) {
        this.yinlian = this.oldUrl.yinlian;
        // 获取公司入款第一个支付
        this.$GlobalService.globalQueryModel.getPayType.payChannel = this.yinlian[0].paychannel;
  ...
  changeType(event, index) {
    ...
    this.$GlobalService.globalQueryModel.heepay.paynum = event.number;
    this.$GlobalService.globalQueryModel.heepay.payPlatFormId = event.payPlatFormId;
  }
  changeAmount(event) {
    this.discount();
  }
  ...
  // 选择优惠
  discount() {
    // console.log(this.$GlobalService.globalQueryModel.heepay.amount);
    // this.$GlobalService.globalQueryModel.getAppDiscount.amount = this.$GlobalService.globalQueryModel.heepay.amount
