...
@Component({
  ...
  styleUrls: ['./online-deposit.component.scss']
})
export class OnlineDepositComponent implements OnInit, DoCheck {
  oldUrl = null;
  onLine;
  isCompany = 0;
  isDiscounts;
  ...
  ngOnInit() {
    this.paychannel = this.$ActivatedRoute.snapshot.paramMap.get('paychannel');
  ...
  ngDoCheck(): void {
    ...
    if (this.oldUrl !== this.$GlobalService.globalQueryModel.getPayListData) {
      ...
      if (Object.keys(this.oldUrl).length !== 0) {
        this.onLine = this.oldUrl.onLine;
        // 获取公司入款第一个支付
        // this.$GlobalService.globalQueryModel.getPayType.payChannel = this.onLine[0].paychannel;
        this.$GlobalService.globalQueryModel.getPayType.payChannel = this.paychannel;
        if (this.paychannel === 'erc20' || this.paychannel === 'trc20' || this.paychannel === 'omni' || this.paychannel === 'cnyb') {
          this.isUSDT = true;
          this.$GlobalService.globalQueryModel.heepay.paychannel = this.paychannel;
        } else {
          this.$GlobalService.globalQueryModel.heepay.paychannel = null;
        }
        this.getPayType();
        setTimeout(() => {
  ...
  companyList(item, i) {
    ...
    if (item.paychannel === 'erc20' || item.paychannel === 'trc20') {
      this.isUSDT = true;
    } else {
      this.isUSDT = false;
    }
    this.$GlobalService.globalQueryModel.getPayType.payChannel = item.paychannel;
    if (item.paychannel === 'erc20' || item.paychannel === 'trc20' || item.paychannel === 'omni' || item.paychannel === 'cnyb') {
      this.$GlobalService.globalQueryModel.heepay.paychannel = item.paychannel;
    } else {
      this.$GlobalService.globalQueryModel.heepay.paychannel = null;
    }
    this.isCompany = i;
