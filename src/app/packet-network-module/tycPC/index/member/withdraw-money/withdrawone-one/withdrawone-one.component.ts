...
export class WithdrawoneOneComponent implements OnInit {
  ...
  constructor(private $CommonDataService: CommonDataService,
    ...
    this.$GlobalService.globalQueryModel.auditwithdrawontime.money = '';
  }
  ...
  getQkVale(event) {
    this.$GlobalService.globalQueryModel.auditwithdrawontime.money = event.target.value;
    if (this.$GlobalService.globalQueryModel.auditwithdrawontime.money) {
      if (this.WITFLOOR > parseInt(this.$GlobalService.globalQueryModel.auditwithdrawontime.money, 10)
        || parseInt(this.$GlobalService.globalQueryModel.auditwithdrawontime.money, 10) > this.WITCEILING) {
        this.isAccuntOK = false;
        this.isnumOks = false;
      } else if(this.newPassworld.length==6){
  ...
  checkPassword(event){
    this.$GlobalService.globalQueryModel.auditwithdrawontime.userbankpassword = event.target.value;
    if (this.$GlobalService.globalQueryModel.auditwithdrawontime.money && this.newPassworld.length==6) {
      this.isnumOks = true;
    } else {
      this.isnumOks = false;
    }
    ...
    this.$CommonDataService.checkFirstWithdrawal().then((res: Result) => {
      if (res.success === 0) {
        if (res.data.isFirstWithdrawals) {
          ...
          saveSet.userbankpassword = this.newPassworld;
  ...
  astscy() {
    ...
    // 取款提交申请
    this.$CommonDataService.auditwithdrawontime().then((res: Result) => {
      this.moneyLoad = !this.moneyLoad;
      if (res.success !== 1) {
        
        //打码量大于0时不足
        this.camamount = res.data.camamount;
        //手续费大于0时扣手续费
        this.cserviceCharge = res.data.serviceCharge;
        // console.log("111111111",this.camamount,this.cserviceCharge)
  ...
  saveDataInput() {
    this.saveLoad = true;
    const saveSet = this.$GlobalService.globalQueryModel.saveWithdraw;
    saveSet.inputmoney = this.$GlobalService.globalQueryModel.auditwithdrawontime.money;
