...
export class LotteryComponent implements OnInit {
  ...
  gameLogin(item) {
    // gameurl, gamecode
    // 维护
    if (parseInt(this.$GlobalService.globalJavaResData.cpwhStatus, 10) !== 0) {
      this.modal.open({
        message: '游戏正在维护！',
        confirmShow: true,
        confirmTxt: '确认',
    ...
    // 试玩
    if (this.$GlobalService.globalQueryModel.swToken && parseInt(this.$GlobalService.globalJavaResData.cpswStatus, 10) === 1 && item.swurl) {
      ...
      return;
    } else if (this.$GlobalService.globalQueryModel.swToken && parseInt(this.$GlobalService.globalJavaResData.cpswStatus, 10) !== 1) { // 状态不为1提示登录
      this.$GlobalMethodsService.showTopCenter();
      // this.$GlobalService.globalTipsModelModel.loginisState.isLoginCon = true;
      this.$GlobalMethodsService.loginOPenWindow();
      return;
    ...
    // 停押
    if (this.$GlobalService.globalQueryModel.userToken && this.$GlobalService.globalJavaResData.cpchargeStatus !== 0) {
      this.modal.open({
