...
export class CompanysPaymentComponent implements OnInit, DoCheck {
  ...
  ngOnInit() {
    ...
    setTimeout( ()=>{
      if(this.underLineName){
        let item = this.underLineName[0]
        // 网银储蓄卡 start
        if (item && item.WYCXK === 'wycxk') {
          this.companyList(item.paychannel,0,item)
          // return;
        } else {
          this.isWYCXK = false;
        }
  ...
  discount(mach) {
    ...
    console.log(this.$GlobalService.globalQueryModel.getPayType.payChannel);
  ...
  nextCompany() {
    ...
    if (this.panyNameCode.bigType === '3') {
      ...
      this.$CommonDataService.getBanklist().then((res: Result) => {
        this.isLoad = !this.isLoad;
        this.serialNumber = 1;
        // 控制tab切换 转账indexShow =2 扫码 indexShow =1
        if (this.$GlobalService.globalQueryModel.getBanklist.banktype !== '0') {
          this.indexShow = 2;
        } else {
          this.indexShow = 1;
        }
  ...
  nextName(moneyValue) {
    ...
    if (this.limitType === 1 || this.isCode) {
