...
export class OnlineDepositComponent implements OnInit, DoCheck {
  ...
  ngDoCheck(): void {
    ...
    if (this.oldUrl !== this.$GlobalService.globalQueryModel.getPayListData) {
      ...
      if (Object.keys(this.oldUrl).length !== 0) {
        ...
        if (this.onLine[0].paychannel === 'erc20' || this.onLine[0].paychannel === 'trc20' || this.onLine[0].paychannel === 'omni') {
          this.isUSDT = true;
          this.$GlobalService.globalQueryModel.heepay.paychannel = this.onLine[0].paychannel;
        } else {
  ...
  companyList(item, i) {
    ...
    if (item.paychannel === 'erc20' || item.paychannel === 'trc20' || item.paychannel === 'omni') {
